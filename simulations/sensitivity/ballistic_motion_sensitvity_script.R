...
#parallized for 100 grams to increase speed
...
for(i in 1:nrow(param_grid)){
  ...
  #save results
  save(patch_results, file = "sim_results/kcal_per_patch/30000g.Rda")
  toc(log = TRUE)
}
...
ggplot(patch_results_df, aes(x = calories, y = offspring)) +
  ...
  geom_vline(xintercept = 2.2, linetype = "dashed", linewidth = 0.5, col = "grey25") +
  scale_x_continuous(
    breaks = c(seq(0, 5, by = 1), 2.2),
    labels = function(x) ifelse(x == 2.2, "2.2", x)) +
  theme.qel()

ggsave(file = "sim_results/kcal_per_patch/figures/30000g.png", width = 6.86, height = 3.5, units = "in", dpi = 600, bg = "white")
